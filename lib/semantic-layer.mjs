...
const DEFAULT_DIMENSION_ALIASES = {
  region: ["地区", "区域", "大区"],
  channel: ["渠道", "来源", "流量渠道"],
  city: ["城市"],
  category: ["品类", "类别", "分类"],
...
export class SemanticLayer {
  ...
  registerModel(input = {}) {
    ...
    for (const column of dimensionColumns.filter((item) => item !== timeColumn)) {
      ...
      dimensions[column] = { label: column, column, type: physical?.numeric ? "number" : "string", aliases: DEFAULT_DIMENSION_ALIASES[column] || [] };
    }
  ...
  matchModelMetrics(model, lower) {
    ...
    return new Set(specific.map((match) => match.key));
  }

  findMetrics(text, modelId = this.activeModelId) {
    const model = this.modelFor(modelId);
    const keys = this.matchModelMetrics(model, text.toLowerCase());
    return Object.keys(model.metrics).filter((key) => keys.has(key));
  }

  findMetricsAcrossModels(text) {
    const lower = text.toLowerCase();
    const matches = [];
    for (const [modelId, model] of this.models) {
      for (const [key, metric] of Object.entries(model.metrics || {})) {
        const terms = [...new Set([key, metric.label, ...(metric.aliases || [])].map((name) => String(name).toLowerCase()).filter(Boolean))];
        for (const term of terms) {
          let start = lower.indexOf(term);
          while (start >= 0) {
            matches.push({ modelId, modelLabel: model.label, modelActive: modelId === this.activeModelId, key, label: metric.label, term, start, end: start + term.length });
            start = lower.indexOf(term, start + 1);
          }
        }
      }
    }
    // 跨全部模型做位置消歧：较长术语覆盖重叠的较短术语，避免「单量」误吞「销售单量」。
    const specific = matches.filter((match) => !matches.some((other) => (other.modelId !== match.modelId || other.key !== match.key)
      && other.term.length > match.term.length && other.start <= match.start && other.end >= match.end));
    const seen = new Set();
    const out = [];
    for (const match of specific) {
      const id = `${match.modelId}:${match.key}`;
      if (seen.has(id)) continue;
      seen.add(id);
      out.push({ modelId: match.modelId, modelLabel: match.modelLabel, modelActive: match.modelActive, key: match.key, label: match.label });
    }
    return out;
  }

  findDimensions(text, modelId = this.activeModelId) {
    const model = this.modelFor(modelId);
    const lower = text.toLowerCase();
    return Object.entries(model.dimensions)
