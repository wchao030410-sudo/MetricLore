...
export class ToolRegistry {
  ...
  createDefinitions() {
    return {
      ...
      semantic_catalog: {
        description: "在受治理语义目录中发现指标和维度候选；可指定语义模型。",
        inputSchema: objectSchema({ query: { type: "string" }, modelId: { type: "string" } }, ["query"]),
      },
      metric_query: {
        description: "执行受控指标查询。禁止传入 SQL，只可传递已注册指标、维度、日期和筛选条件。",
        inputSchema: objectSchema({ metrics: { type: "array", items: { type: "string" }, minItems: 1 }, dimensions: { type: "array", items: { type: "string" } }, filters: { type: "object" }, range: objectSchema({ startDate: { type: "string" }, endDate: { type: "string" } }, ["startDate", "endDate"]), timeGrain: { type: "string", enum: ["day", "week", "month"] }, modelId: { type: "string" } }, ["metrics", "range"]),
      },
      compare_periods: {
        description: "对同一个注册指标在两个期间做确定性聚合对比。",
        inputSchema: objectSchema({ metric: { type: "string" }, currentRange: objectSchema({ startDate: { type: "string" }, endDate: { type: "string" } }, ["startDate", "endDate"]), baselineRange: objectSchema({ startDate: { type: "string" }, endDate: { type: "string" } }, ["startDate", "endDate"]), dimensions: { type: "array", items: { type: "string" } }, filters: { type: "object" }, modelId: { type: "string" } }, ["metric", "currentRange", "baselineRange"]),
      },
      dimension_breakdown: {
        description: "按照一个已注册维度拆分注册指标。",
        inputSchema: objectSchema({ metric: { type: "string" }, dimension: { type: "string" }, range: objectSchema({ startDate: { type: "string" }, endDate: { type: "string" } }, ["startDate", "endDate"]), filters: { type: "object" }, limit: { type: "integer", minimum: 1, maximum: 50 }, modelId: { type: "string" } }, ["metric", "dimension", "range"]),
      },
      submit_evidence: {
  ...
  async execute(name, args) {
    ...
    else if (name === "semantic_catalog") data = this.catalog(args);
  ...
  catalog({ query, modelId }) {
    const metrics = this.semantic.findMetrics(query, modelId);
    const dimensions = this.semantic.findDimensions(query, modelId);
    const model = modelId ? this.semantic.modelFor(modelId) : this.semantic.model;
    return {
      metrics: metrics.map((key) => ({ key, ...model.metrics[key] })),
      dimensions: dimensions.map((key) => ({ key, ...model.dimensions[key] })),
  ...
  metricQuery(args) {
    ...
    const result = this.semantic.execute(this.db, { metrics: args.metrics, dimensions: args.dimensions || [], filters: args.filters || {}, startDate: range.startDate, endDate: range.endDate, timeGrain: args.timeGrain || null, modelId: args.modelId });
    return { rows: result.rows, metrics: result.metrics, dimensions: result.dimensions, timeGrain: result.timeGrain, scope: { modelId: args.modelId || result.modelId, range, filters: args.filters || {}, rowCount: result.rows.length } };
  }

  comparePeriods(args) {
    const currentRange = dateRange(args.currentRange);
    const baselineRange = dateRange(args.baselineRange);
    const input = (range) => this.semantic.execute(this.db, { metrics: [args.metric], dimensions: args.dimensions || [], filters: args.filters || {}, startDate: range.startDate, endDate: range.endDate, modelId: args.modelId });
    ...
    return { metric: key, rows, scope: { modelId: args.modelId || null, currentRange, baselineRange, dimensions: args.dimensions || [], filters: args.filters || {} } };
  }

  dimensionBreakdown(args) {
    const result = this.metricQuery({ metrics: [args.metric], dimensions: [args.dimension], filters: args.filters || {}, range: args.range, modelId: args.modelId });
