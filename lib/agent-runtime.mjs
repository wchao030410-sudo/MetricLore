...
export class AgentRuntime {
  ...
  async run(message, history = [], context = {}) {
    ...
    const trace = [{ state: "RECEIVED", at: Date.now() }, { state: "RESOLVING_CAPABILITY", capability: resolution.capability, at: Date.now() }, { state: "SELECTING_SKILL", skill: skill.name, at: Date.now() }, { state: "PLANNING", goal: plan.goal, steps: plan.steps.map((step) => step.tool), at: Date.now() }];
    ...
    return {
      ...
      toolCalls: calls.map(({ name, args, result }) => ({ name, args, scope: result.scope, status: result.status })),
      plan,
