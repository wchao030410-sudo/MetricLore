...
export class AgentRuntime {
  ...
  async run(message, history = []) {
    ...
    const evidenceRecord = await invoke("submit_evidence", { claims: [answer.slice(0, 500)], sourceKeys: [...new Set(sources.map((item) => item.key).filter(Boolean))] });
    trace.push({ state: "VALIDATING", at: Date.now() }, { state: "COMPLETED", at: Date.now() });
    const status = resolution.capability === "safety" ? "refused" : sources.length ? "verified" : "not_answerable";
    return { answer, status, skill: skill.name, mode: resolution.capability, provider, sources, data: this.dataFromCalls(calls), evidence: evidenceRecord.data, publicTrace: trace, toolCalls: calls.map(({ name, args, result }) => ({ name, args, scope: result.scope, status: result.status })) };
  }
