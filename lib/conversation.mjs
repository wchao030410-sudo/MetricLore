...
export class ConversationService {
  ...
  completeRun(runId, assistantMessageId, conversationId, context, result) {
    ...
    this.db.prepare("UPDATE agent_runs SET status = 'completed', capability = ?, provider = ?, context_after_json = ?, plan_json = ?, budget_json = ?, validation_json = ?, error_json = NULL, completed_at = ? WHERE id = ?")
      .run(result.mode, result.provider, json(after), json(result.plan), json(result.plan?.budget || {}), json(result.validation), now, runId);
    this.db.prepare("UPDATE conversations SET updated_at = ?, last_message_at = ? WHERE id = ?").run(now, now, conversationId);
  ...
  async resolveClarification(runId, { optionId, defer = false } = {}) {
    ...
    if (!normalizedOptionId || !clarification?.options?.some((option) => option.id === normalizedOptionId)) throw new Error("澄清选项无效");

    const userMessage = this.getMessage(run.userMessageId);
    const context = this.loadContext(run.conversationId);
    const next = this.mergeContext(context, { metrics: [normalizedOptionId], pendingClarification: null });
    ...
    this.db.prepare("UPDATE agent_runs SET status = 'queued', capability = NULL, provider = NULL, context_before_json = ?, context_after_json = NULL, plan_json = NULL, budget_json = NULL, validation_json = NULL, error_json = NULL, completed_at = NULL WHERE id = ?")
      .run(json(next), runId);
    this.appendEvent(runId, "clarification.resolved", { optionId: normalizedOptionId });
