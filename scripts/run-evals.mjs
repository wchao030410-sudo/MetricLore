...
const html = `<!doctype html><meta charset="utf-8"><title>Data Agent Eval Report</title><style>body{font-family:system-ui;margin:40px;color:#17221d}table{border-collapse:collapse;width:100%;font-size:13px}th,td{border:1px solid #dfe7e1;padding:8px;text-align:left}.pass{color:#0d7a50}.fail{color:#b42318}</style><h1>Data Agent Evaluation</h1><p>Generated: ${escape(report.generatedAt)}</p><h2>${passed}/${results.length} passed (${(report.passRate * 100).toFixed(1)}%)</h2><p>Three-run public-result consistency: ${(report.consistencyRate * 100).toFixed(1)}%</p><table><tr><th>ID</th><th>Group</th><th>Question</th><th>Skill</th><th>Tools</th><th>Result</th></tr>${results.map((item) => `<tr><td>${escape(item.id)}</td><td>${escape(item.group)}</td><td>${escape(item.question)}</td><td>${escape(item.skill)}</td><td>${escape(item.tools.join(", "))}</td><td class="${item.pass ? "pass" : "fail"}">${item.pass ? "PASS" : escape(item.failures.join("; "))}</td></tr>`).join("")}</table>`;
