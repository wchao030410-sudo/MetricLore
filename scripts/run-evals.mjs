...
const agent = new DataAgent({ semantic, wiki, db, ontology, skills: new SkillRegistry() });
const results = [];
for (const item of cases) {
  const result = await agent.answer(item.question);
  ...
  results.push({ id: item.id, group: item.group, question: item.question, pass: failures.length === 0, failures, skill: result.skill, status: result.status, tools });
}
closeDatabase();
const passed = results.filter((item) => item.pass).length;
const report = { generatedAt: new Date().toISOString(), caseCount: results.length, passed, failed: results.length - passed, passRate: passed / results.length, results };
...
const html = `<!doctype html><meta charset="utf-8"><title>Data Agent Eval Report</title><style>body{font-family:system-ui;margin:40px;color:#17221d}table{border-collapse:collapse;width:100%;font-size:13px}th,td{border:1px solid #dfe7e1;padding:8px;text-align:left}.pass{color:#0d7a50}.fail{color:#b42318}</style><h1>Data Agent Evaluation</h1><p>Generated: ${escape(report.generatedAt)}</p><h2>${passed}/${results.length} passed (${(report.passRate * 100).toFixed(1)}%)</h2><table><tr><th>ID</th><th>Group</th><th>Question</th><th>Skill</th><th>Tools</th><th>Result</th></tr>${results.map((item) => `<tr><td>${escape(item.id)}</td><td>${escape(item.group)}</td><td>${escape(item.question)}</td><td>${escape(item.skill)}</td><td>${escape(item.tools.join(", "))}</td><td class="${item.pass ? "pass" : "fail"}">${item.pass ? "PASS" : escape(item.failures.join("; "))}</td></tr>`).join("")}</table>`;
writeFileSync(resolve(ROOT, "outputs/evals/latest.html"), html);
console.log(JSON.stringify({ caseCount: report.caseCount, passed, failed: report.failed, passRate: report.passRate, output: "outputs/evals/latest.json", html: "outputs/evals/latest.html" }, null, 2));
